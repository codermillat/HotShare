...
| | |
|---|---|
| **How it works** | The host phone runs an HTTP (`:8080`) + SOCKS5 (`:1080`) proxy bound to its hotspot IP. Clients (Android / macOS / any OS) route their traffic through it, so every connection is **authenticated and re-originated by the host** — clients never need direct outbound access. |
...
- [`SHARING-GUIDE.md`](SHARING-GUIDE.md) — host/client setup and troubleshooting (band selection, proxy config, QR pairing, `ping` caveats)
- [`ACCEPTABLE-USE.md`](ACCEPTABLE-USE.md) — intended use, provider terms, contribution policy
...
## Status
Last device-verified build: **v1.5** — end-to-end on real devices (Samsung / OnePlus / TECNO clients, Android 13–16). **v1.6** raises `targetSdk` to 36 for Google Play and needs a fresh two-device pass. See [`HotShare/dist/TEST-REPORT.md`](HotShare/dist/TEST-REPORT.md).
